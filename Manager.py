    ...
    def handle_complaint(self):
        ...
        while complaint_id != 0:
            try:
                ...
                self.handle_complaint_reply(complaint)
            except IndexError:
                print("输入'complaint_id'错误，请重新输入：\n")
                continue
            complaint_id = int(input("请根据'complaint_id'选择投诉，退出请输入'0'：\n>>>"))

    def handle_complaint_reply(self, complaint: Complaint):
...
if __name__ == '__main__':
    ...
    manager_control_lst = ['manager.handle_complaint', 'manager.close_complaint', ]
